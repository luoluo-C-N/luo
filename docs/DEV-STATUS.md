...
| v0.11-15 | 钱包UI+同步骨架+相册骨架+集成测试+暗色快捷+骨架屏+重试 |
| 测试 | 126/126 全绿 |
| 安全 | rate limiter + sanitizeInput + 剪贴板15s清除 已集成 |
| Vault UI | pg-account 三态切换（设置/解锁/锁定）已就绪 |
| 密码 UI | 列表/搜索/添加(生成器+强度条)/复制15s清除/删除/分类折叠 已就绪 |
| 钱包 UI | 月度汇总+图表+趋势+分类占比+记一笔 已就绪 |
| 通知 | pg-notif 渲染待审数据已就绪 |
| 相册 | 照片网格骨架已就绪（上传待后端） |
| 版本目标 | v0.40 已达成 |
| 旧测试计数 | 121/121 全绿 |
| 版本目标 | v0.40 已达成 |
